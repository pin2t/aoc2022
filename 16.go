...
func pressure(initial state, forbidden []int) int {
	result := 0
	processed := vset{}
	queue := []state{}
	...
	for len(queue) > 0 {
		s := queue[0]
		queue = queue[1:]
		if processed.contains(s.pressure*26*26+s.valve) || s.timeLeft == 0 {
			continue
		}
		processed[s.pressure*26*26+s.valve] = true
		result = max(result, s.pressure)
		if !s.opened.contains(s.valve) && !contains(forbidden, s.valve) && flow[s.valve] > 0 {
			queue = append(queue, s.open())
		}
		for to, _ := range flow {
			if to != s.valve && !contains(forbidden, to) && tunnels.contains(s.valve*26*26+to) {
...
func powerset(original []int) [][]int {
	size := int(math.Pow(2, float64(len(original))))
	result := [][]int{}
	var index int
	for index < size {
		var subSet []int
...
func main() {
	...
	n1 := pressure(state{0, vset{}, 30, 0}, []int{})
	n2 := 0
	for _, first := range powerset(nonempty) {
		second := []int{}
		for v, f := range flow {
			if !contains(first, v) && f > 0 {
